...
class BasePaymentService:
    ...
    def _payment_init_response(
        *, order_id: str, link: str, amount_sum: int, quote: dict | None
        ...
        response = {"order_id": order_id, "link": link, "amount_sum": amount_sum}
        if isinstance(quote, dict):
            for key in ("full_amount_sum", "upgrade_credit_sum", "upgrade_from_tier"):
                if key in quote:
        ...
        *,
        ...
        if existing_invoice:
            if (
                ...
                return self._payment_init_response(
                    order_id=order_id_value,
                    link=link,
                    amount_sum=int(existing_invoice.get("amount_sum") or amount_sum),
                    quote=existing_invoice.get("subscription"),
                )
