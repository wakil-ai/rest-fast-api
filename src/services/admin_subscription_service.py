...
Errors use main's existing convention (see ``SubscriptionEligibilityError.to_detail()``
in ``models.payment``): a plain ``HTTPException`` with a dict-shaped ``detail``
carrying ``code``/``message``/params, not a separate coded-error-envelope layer.
"""
...
def _admin_error(http_status: int, code: str, message: str, **params: Any) -> HTTPException:
    """A coded failure on the admin subscription surface.

    Kept local rather than a shared catalog: this is the only place on ``main``
    that needs a structured admin error today, and every raise site already names
    its own status/message/params, so a catalog would just be indirection.
    """
    return HTTPException(status_code=http_status, detail={"code": code, "message": message, **params})
        ...
        if request.target == "daily_lot":
            ...
            if lot is None:
                raise _admin_error(
                    status.HTTP_404_NOT_FOUND,
                    "DAILY_LOT_NOT_FOUND",
                    "No daily pass lot with that id.",
                    daily_lot_id=request.daily_lot_id,
                )
            new_end = (
        ...
        *,
        ...
        try:
            ...
            await self.audit_service.finalize(
                event_id, result="error", before=before, error_code=error_code
            )
            raise
        except SubscriptionEligibilityError as exc:
            await self.audit_service.finalize(
