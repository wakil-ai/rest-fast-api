...
def _org_to_response(doc: dict) -> OrganizationResponse:
...
async def create_organization(user_id: str, body: OrganizationCreateRequest):
...
async def list_organizations(user_id: str):
    ...
    return OrganizationListResponse(
        organizations=[_org_to_response(d) for d in docs]
    )


def _invite_to_response(doc: dict) -> OrganizationInviteResponse:
    ...
    return OrganizationInviteResponse(
        ...
        created_at=doc["created_at"],
        join_path=doc.get("join_path") or member_service._invite_join_path(invite_id),
    )
...
@handle_service_error
async def create_organization_invite(org_id: str, user_id: str):
...
@handle_service_error
async def list_organization_invites(org_id: str, user_id: str):
...
@handle_service_error
async def revoke_organization_invite(org_id: str, invite_id: str, user_id: str):
...
@handle_service_error
async def preview_organization_invite(invite_id: str, user_id: str):
...
async def accept_organization_invite(
    invite_id: str, body: OrganizationInviteAcceptRequest
):
...
@handle_service_error
async def list_organization_members(org_id: str, user_id: str):
