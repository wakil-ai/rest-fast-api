...
- Payme, Click, Uzum, Atmos first charge: `BasePaymentService._finalize_subscription_invoice`
- Atmos renewals: `charge_mandate` (a fresh order id per charge, so each renewal is its own event)
...
- **Payme/Click/Uzum/Atmos:** the catalog `amount_sum`, always UZS, sent as `UZS` without conversion.
