...
def response_error_code(response) -> str:
    """This branch has no coded-error-envelope layer: read `detail.code` directly."""
...
def test_mutations_require_an_operator(client, super_admin_key, route, payload):
    ...
    assert response_error_code(response) == "ADMIN_OPERATOR_REQUIRED"
...
def test_mutations_require_a_uuid_request_id(client, super_admin_key, route, payload):
    ...
    assert response.status_code == 400
    assert response_error_code(response) == "ADMIN_REQUEST_ID_REQUIRED"
...
def test_replaying_a_request_id_does_not_apply_twice(client, super_admin_headers):
    ...
    assert response_error_code(second) == "ADMIN_ACTION_DUPLICATE"
...
def test_a_failed_action_is_still_audited(client, super_admin_headers):
    ...
    assert response_error_code(response) == "SUBSCRIPTION_NOT_FOUND"
