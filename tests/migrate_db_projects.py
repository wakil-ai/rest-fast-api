...
async def migrate_ids_and_projects():
    ...
    for col_name, id_field in target_collections.items():
        ...
        cursor = collection.find({"_id": {"$type": "objectId"}, id_field: {"$exists": True}})
            ...
            for doc in docs_to_migrate:
                try:
                    ...
                    if collection.find_one({"_id": target_id}):
                        print(f"    SKIPPING: Document with _id={target_id} already exists. Deleting duplicate old ObjectId doc.")
                        ...
                        try:
                            ...
                            print(f"    Error deleting duplicate old doc {old_oid}: {e}")
                        continue
                    ...
                    collection.update_one(
                        {"_id": old_oid}, 
                        {"$set": {id_field: temp_val}}
                    )

                    # 2. Prepare new document
                    new_doc = doc.copy()
                    new_doc["_id"] = target_id
                    new_doc[id_field] = target_id # Ensure it has the correct original value
                    ...
                    try:
                        ...
                        print(f"    Insert failed for {target_id}: {insert_error}. Reverting rename.")
                        collection.update_one(
                            {"_id": old_oid}, 
                            {"$set": {id_field: target_id}}
                        )
                        error_count += 1
                        continue
