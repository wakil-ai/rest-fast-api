...
class _DailyLotCollection:
    ...
    def find(self, query: dict):
        docs = [
            ...
            and doc["end_ms"] > query["end_ms"]["$gt"]
            and doc["credits_remaining"] > query["credits_remaining"]["$gt"]
        ]
        return _Cursor(docs)
    ...
    async def find_one_and_update(self, query: dict, update: dict, return_document=True):
        for doc in self.docs:
            if (
                doc["_id"] == query["_id"]
                and doc["user_id"] == query["user_id"]
                and doc["end_ms"] > query["end_ms"]["$gt"]
                and doc["credits_remaining"] >= query["credits_remaining"]["$gte"]
            ):
