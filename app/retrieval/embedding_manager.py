...
class QwenEmbedding(BaseEmbedding):
    ...
    def rerank(self, query: str, documents: List[str], top_k: int = settings.TOP_K) -> List[dict]:
        payload = {
        ...
        try:
            response = requests.post(self.rerank_url, headers=self.headers, json=payload)
    ...
    def embed_query(self, query: str) -> List[float]:
        return self.embed_doc(get_instruction(query))  # Qwen doesn't use instructions for queries
...
class EmbeddingManager:
    ...
    def __init__(self):
        ...
        logger.info(f"EmbeddingManager initialized with {settings.EMBEDDING_MODEL} model")
