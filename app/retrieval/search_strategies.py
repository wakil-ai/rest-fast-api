...
class SearchStrategy:
    ...
    def _search_mamuriy_sud(
        self, query: str, config: RetrievalConfig, expr: Optional[str] = None, top_k_file: int = 3
    ) -> list[dict[str, Any]]:
        ...
        for doc in search_results:
            ...
            searched_file_results.sort(key=lambda x: x.get("metadata", {}).get("chunk_id", 0))
            file_texts = file_texts = "\n".join(
                file_doc.get("text", "") for file_doc in searched_file_results
            )
            file_texts = f"File Title: {hierarchy}\n\n{file_texts}"
            documents.append({"text": file_texts, "metadata": {"file_id": file_id, "file_name": file_name, "text": file_texts, "hierarchy": hierarchy}})
