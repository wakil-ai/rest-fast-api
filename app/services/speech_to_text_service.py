            ...
            adaptation = self.speech.SpeechAdaptation(
                phrase_sets=[
                    self.speech.SpeechAdaptation.PhraseSet(
                        phrases=[
                            self.speech.SpeechAdaptation.PhraseSet.Phrase(value=hint)
                            for hint in hints
                        ]
                    )
                ]
...
class AzureSpeechToTextService(SpeechToTextService):
    ...
    def transcribe_audio(
        ...
        if hints:
            phrase_list_grammar = self.speechsdk.PhraseListGrammar.from_recognizer(
                speech_recognizer
            )
            for hint in hints:
...
class AzureRESTSpeechToTextService(SpeechToTextService):
    ...
    def transcribe_audio(
        ...
        try:
            ...
            url = (
                f"{self.base_url}/speech/recognition/conversation/cognitiveservices/v1"
            )
            params = {"language": language, "format": "detailed"}

            # Make the request
            response = requests.post(
                url, headers=headers, params=params, data=audio_data, timeout=60
            )
...
def _is_running_in_docker() -> bool:
    ...
    return (
        ...
        or os.environ.get("PYTHONPATH") == "/app"
    )
...
def get_speech_to_text_service() -> SpeechToTextService:
    ...
    if settings.SPEECH_TO_TEXT_PROVIDER == "google":
        ...
        raise ValueError(
            f"Unknown speech-to-text provider: {settings.SPEECH_TO_TEXT_PROVIDER}"
        )
