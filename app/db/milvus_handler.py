...
class MilvusHandler(VectorDBHandler):
    ...
    def _create_schema(self):
        ...
        schema.add_field(field_name="hierarchy_path", datatype=DataType.VARCHAR, max_length=65535, enable_analyzer=True, default_value="None in hierarchy_path")
        schema.add_field(field_name="text_dense", datatype=DataType.FLOAT_VECTOR, dim=settings.EMBEDDING_DIM)
        schema.add_field(field_name="text_sparse", datatype=DataType.SPARSE_FLOAT_VECTOR)
        schema.add_field(field_name="text_sparse_hierarchy", datatype=DataType.SPARSE_FLOAT_VECTOR)
    ...
    def build_like_or_expr(self, values: Iterable[Union[int, str]]) -> str:
        """
