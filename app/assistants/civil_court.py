...
class CivilCourtAssistant(BaseAssistant):
    ...
    async def _retrieve_general(
        ...
        civ_config = RetrievalConfig(
            top_k=settings.TOP_K,
            collection_name=settings.MILVUS_CIVIL_COURT,
            filter=filter,
        )
        civ_docs = await self.asearch(effective, civ_config)
        civ_result = await self.format_results(civ_docs)
        ...
        combined_ctx = f"{civ_result.context}\n\n{'=' * 60}\n\n{main_result.context}"
        if file_context:
            combined_ctx += f"\n\n\n{file_context}"

        return RetrievalResult(
            context=combined_ctx,
            attachments=civ_result.attachments + main_result.attachments,
        )
